...
export async function saveProject(formData: FormData) {
  ...
  // 제출이 닫힌 뒤의 수정 차단. 심사가 진행되는 중에 제출물이 바뀌면 채점
  // 근거가 흔들리므로, 화면 숨김이 아니라 여기서 막아야 한다(RLS 로도 이중 방어).
  const { data: settings } = await supabase
    .from("event_settings")
    .select("project_submit_open")
    .single();
  if (!canSubmitProject(settings?.project_submit_open))
