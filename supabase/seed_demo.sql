...
-- 주민 투표는 별도 테이블(audience_votes)에 쌓이므로 여기서 넣을 값이 없다(0045).
insert into projects (
  id, team_id, title, description,
  repo_url, demo_url, video_url, deck_url,
  present_order, view_count, submitted_at
)
values
  ('d1000000-0000-4000-8000-000000000001', 'd0000000-0000-4000-8000-000000000001',
   ...
   1, 142, timestamptz '2026-09-18 14:05:00+09'),
  ...
  ('d1000000-0000-4000-8000-000000000002', 'd0000000-0000-4000-8000-000000000002',
   ...
   2, 98, timestamptz '2026-09-18 14:12:00+09'),
  ...
  ('d1000000-0000-4000-8000-000000000003', 'd0000000-0000-4000-8000-000000000003',
   ...
   3, 76, timestamptz '2026-09-18 14:20:00+09'),
  ...
  ('d1000000-0000-4000-8000-000000000004', 'd0000000-0000-4000-8000-000000000004',
   ...
   4, 121, timestamptz '2026-09-18 14:28:00+09'),
  ...
  ('d1000000-0000-4000-8000-000000000005', 'd0000000-0000-4000-8000-000000000005',
   ...
   5, 64, timestamptz '2026-09-18 14:36:00+09'),
  ...
  ('d1000000-0000-4000-8000-000000000006', 'd0000000-0000-4000-8000-000000000006',
   ...
   6, 53, timestamptz '2026-09-18 14:44:00+09'),
  ...
  ('d1000000-0000-4000-8000-000000000007', 'd0000000-0000-4000-8000-000000000007',
   ...
   7, 88, timestamptz '2026-09-18 14:52:00+09'),
  ...
  ('d1000000-0000-4000-8000-000000000008', 'd0000000-0000-4000-8000-000000000008',
   ...
   null, null,
   8, 41, timestamptz '2026-09-18 15:00:00+09')
on conflict do nothing;

-- ---------- 주제(트랙) ----------
-- 갤러리 주제 필터를 실제로 눌러볼 수 있도록 데모 제출작에 주제를 붙인다.
--
-- 주제 어휘는 projects_track_chk 가 강제한다. 0032 의 첫 분류(mobility·
-- education)는 0038 → 0042 를 거치며 공고의 5개 분야로 바뀌었는데 이 시드만
-- 옛 값을 들고 있어 제약 위반으로 막혔다. 현재 허용값(0042)만 쓴다:
...
from (values
  ...
  ('d1000000-0000-4000-8000-000000000003'::uuid, 'barrierfree'),  -- 보행 약자 신호 연장
  ('d1000000-0000-4000-8000-000000000004'::uuid, 'esg'),
  ('d1000000-0000-4000-8000-000000000005'::uuid, 'youth'),        -- 재도전 지원 공간
  ('d1000000-0000-4000-8000-000000000006'::uuid, 'esg'),
  ('d1000000-0000-4000-8000-000000000007'::uuid, 'etc'),          -- 민원 소통
  ('d1000000-0000-4000-8000-000000000008'::uuid, 'youth')         -- 귀촌 정착 지원
) as v(id, track)
